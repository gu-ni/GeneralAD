...
score_dir = "/workspace/MegaInspection/GeneralAD/scores"
...
for scenario in scenarios:
    ...
    cases = sorted(cases, key=lambda x: int(x[:-13]))  # 예: case000_001000

    fm_scores_of_dataset_per_case = {}

    for case in cases:
        case_dir = os.path.join(score_dir, scenario, case)
        datasets = sorted(os.listdir(case_dir), key=lambda x: int(x[7:]))  # 예: dataset7
